...
async def derive_persona(state: GraphState) -> dict:
    ...
    # возвращаем её в state: версия в паспорте и рецепте отражает карточку, из
    # которой на ЭТОМ проходе взят блок аудиторий. Факты о продукте (блоки 1-2)
    # заморожены на проходе understand_product: он стоит на START и в цикле
    # перегенерации персоны не участвует, так что после правки карточки на
    # остановке они останутся из прежней версии.
