...
(function (root) {
  ...
  // Обратно-битовый порядок восьми ступеней: 0 4 2 6 1 5 3 7. Упорядоченная
  // последовательность, а не случайный шум: шум даёт грязную кашу, здесь осыпь
  // ложится ровным зерном и читается фактурой, а не помехой. Каждая следующая
  // ступень садится в самую широкую из оставшихся дыр, поэтому зерно остаётся
  // равномерным на любой доле пути, а не сгущается к одному краю.
  var VDC = [0, 4, 2, 6, 1, 5, 3, 7];
  ...
  function drawSeam(cv) {
    ...
    // заявлена в 48, а осыпается в 24, и ширина полосы почти ничего не меняет.
    // Линейная тратит всю полосу целиком.
    ...
    // выходит чистой поверхностью, и осыпь начинается ЗА границей, а не на ней.
    //
    // Порог берётся вдоль ДЛИННОЙ оси, а не из матрицы 8×8, и это главное здесь.
    // Доля пути постоянна внутри колонки, поэтому от матрицы колонка получала
    // ровно один свой столбец — а столбцы у Байера двухгорбые, без середины.
    // Скат из-за этого вырождался в три тона: 16px сплошного, 24px ровной
    // шахматки, 8px сплошного, при том что заявлено 48px распада. Шириной полосы
    // это не лечится — при 64 скат вдобавок перестаёт быть монотонным.
    // Здесь же в колонке зажигается ровно столько клеток, сколько ступеней
    // таблицы меньше доли пути: скат монотонен по построению и живёт всю полосу.
    // Смещение колонки — постоянный шаг 3. Шаг взаимно прост с восьмёркой,
    // поэтому колонки перебирают все ступени и ни одна пара не повторяется.
    // Тройка не на глаз: перебором всех смещений она даёт минимум слипания
    // соседних клеток. Шаг 1 или 7 дал бы лестницу в одну клетку, то есть
    // сплошную диагональ во весь стык, и зерно читалось бы линией.
    // На монотонность смещение не влияет вовсе — оно только тасует, какие
    // именно клетки зажечь, а не сколько.
    for (var y = 0; y < h; y += cell) {
      var gy = Math.floor(y / cell);
      for (var x = 0; x < w; x += cell) {
        var gx = Math.floor(x / cell);
        var t = vertical ? x / w : y / h;
        var a = vertical ? gx : gy;          // шаг вдоль ската
        var b = vertical ? gy : gx;          // шаг вдоль стыка
        if (t > VDC[(b + 3 * a) % 8] / 8) ctx.fillRect(x, y, cell, cell);
      }
    }
  }
