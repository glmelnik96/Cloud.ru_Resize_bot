...
def test_the_seam_ramp_spends_the_whole_band(tmp_path, monkeypatch):
    """Скос обещал 48px распада, а отдавал трёхтоновую полоску.

    Вдоль ската при полосе 48 и зерне 8 всего шесть колонок клеток, и каждая
    брала РОВНО ОДИН столбец матрицы Байера. Столбцы у Байера двухгорбые — во
    втором это .06 .11 .13 .17 и .81 .86 .88 .92, середины нет вовсе, — поэтому
    доли заполнения выходили [0, 0, .5, .5, .5, 1]: 16px сплошного, 24px ровной
    шахматки, 8px сплошного. Два столбца матрицы не опрашивались никогда.

    Шириной полосы это не лечится: при 48 матрице тесно, а при 64 скат ещё и
    теряет монотонность — [0, 0, .5, .25, .5, .75, .5, 1]. Упорядоченная матрица
    8×8 в принципе не даёт ската, когда вдоль него меньше 64 клеток.

    Порог берём из обратно-битового порядка вдоль ДЛИННОЙ оси: клеток в колонке
    становится ровно столько, сколько ступеней таблицы меньше доли пути, то есть
    скат монотонен по построению и тратит полосу целиком."""
    with TestClient(_app(tmp_path, monkeypatch)) as c:
        js = c.get("/static/ascii.js").text
        assert "B8" not in js and "function bayer" not in js
        vdc = [int(n) for n in js.split("var VDC = [", 1)[1].split("]", 1)[0].split(",")]
        assert sorted(vdc) == list(range(8))   # перестановка, а не произвольный список
        draw = js.split("function drawSeam(cv) {", 1)[1].split("\n  }", 1)[0]
        assert "VDC[(b + 3 * a) % 8] / 8" in draw
        # Ось выбирается формой полотна, и вдоль ската идёт a, вдоль стыка — b.
        assert "var a = vertical ? gx : gy;" in draw
        assert "var b = vertical ? gy : gx;" in draw

        # Полоса 48 при зерне 8 — шесть колонок; доля берётся по набегающей кромке.
        def levels(shift):
            return [sum(1 for b in range(8) if a / 6 > vdc[(b + shift * a) % 8] / 8)
                    for a in range(6)]

        assert levels(3) == [0, 2, 3, 4, 6, 7]
        assert levels(3) == sorted(levels(3))  # монотонно: назад скат не идёт
        assert len(set(levels(3))) == 6        # шесть колонок — шесть разных ступеней
        # Первая колонка чистая: осыпь начинается ЗА границей этажа, а не на ней.
        assert levels(3)[0] == 0
        # Смещение колонки тасует, КАКИЕ клетки зажечь, но не сколько — поэтому
        # зерно можно перенастраивать, не трогая сам скат. Тройку выбрали по
        # минимуму слипания соседних клеток, и это решение чисто зерновое.
        assert all(levels(s) == levels(3) for s in range(8))
